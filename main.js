...
function createFfmpegCaptureProcess(myModel) {
  return Promise
    .try(() => {
      var filename = myModel.nm + '_' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.ts';
        ...
        fs.statAsync(`${captureDirectory}/${filename}`)
          .then(stats => {
            if (stats.size <= (config.minFileSizeMb * 1048576)) {
              ...
              //ADD DIR FOR MODEL
              var modelDir = completeDirectory + '/' + myModel.uid + ' - ' + myModel.nm;
              mkdirp(modelDir, (err) => {
                if (err) {
                  printErrorMsg(err);
                } else {
                  printMsg("Dir '" + modelDir + "' created");
                  mv(captureDirectory + '/' + filename, modelDir + '/' + filename, err => {
                    if (err) {
